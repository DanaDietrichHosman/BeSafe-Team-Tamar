...
app.get("/", async (req, res) => {
  try {
    // איחוד מילים זהות וסכימת ה-count שלהן
    ...
    // יצירת שורות הטבלה מהנתונים המאוחדים
    ...
    const html = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 40px auto; padding: 20px; border: 1px solid #ddd; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <h3 style="margin-top: 20px;">Aggregated Offensive Words:</h3>
        <table style="width: 100%; border-collapse: collapse; text-align: left;">
          ...
          <tbody>
            ${rows || '<tr><td colspan="2" style="padding:20px; text-align:center;">No data yet.</td></tr>'}
          </tbody>
        </table>
      </div>
    `;
    ...
    console.error("Dashboard Error:", error); // שימוש במשתנה פותר את שגיאת ה-Lint
    res.status(500).send(`
